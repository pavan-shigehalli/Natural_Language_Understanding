...
and it is modified to fit to our requirements
'''
...
class GloVeModel():
    ...
    def __init__(self, embedding_size, context_size, max_vocab_size=100000, min_occurrences=1,
        ...
        # Log the loss vs iteration
        if self.loss_report_file :
            with open(self.loss_report_file, 'w') as file :
                csv_file = csv.writer(file)
                csv_file.writerow(['Iteration', 'Loss'])


    def fit_to_corpus(self, corpus, save_model=None):
        self.__fit_to_corpus(corpus, save_model, self.max_vocab_size, self.min_occurrences,
                             self.left_context, self.right_context)
        self.__build_graph()


    def __fit_to_corpus(self, corpus, save_model, vocab_size, min_occurrences, left_size, right_size):
        word_counts = Counter()
    ...
    def train(self, num_epochs, log_dir=None, summary_batch_interval=1000,
              tsne_epoch_interval=None, save_model=None):
        ...
        total_steps = 0
        widgets = ['GloVe Training : ', progressbar.Percentage(), progressbar.Bar(),progressbar.ETA()]
        bar = progressbar.ProgressBar(widgets=widgets, maxval=num_epochs).start()
        ...
        with tf.Session(graph=self.__graph) as session:
            ...
            for epoch in range(num_epochs):
                ...
                for batch_index, batch in enumerate(batches):
                    ...
                    __loss,_ = session.run([self.__total_loss,self.__optimizer], feed_dict=feed_dict)
                ...
                # Log the loss vs iteration
                if self.loss_report_file :
                    with open(self.loss_report_file, 'a') as file :
                        csv_file = csv.writer(file)
                        csv_file.writerow([epoch, __loss])
